# Pure Indexing, Flirting with Factor Tilts, and Finding Intermediaries: Thoughts on Addressing Index Inefficencies in the Traditional Three-Fund Portfolio



## Post Rationale

Recently having finished [Andrew Angs](https://www.sipa.columbia.edu/communities-connections/faculty/andrew-ang) book "Asset Management: A Systematic Approach to Factor Investing" and also having rearead the newest 50th edition version of [Burton Malkiels](https://jrc.princeton.edu/people/burton-g-malkiel) "A Random Walk Down Wall Street", I wanted to more deeply think about indexing inefficiencies that low-cost fund combinations of international and US equity ETFs like VTI and VXUS inherently have, and how literature-informed companies like [Dimensional](https://www.dimensional.com/us-en/individual) provide "Market" type funds that address these inefficencies, without me needing to necessarily commit to the idea of factor tilting yet. Thus, the aim here is to essentially still capture market beta (being slightly optimized with higher net returns), where DFUS seems to be a replacement for VTI, but there is no simple replacement for VXUS.

This post is just a string of semi-structured thoughts (by your average retail investor yours truly) that are by definition incomplete and work in progress, so any constructive feedback would be greatly appreciated, especially if you can help solve the dilemma that lead me to write this post in the first place.

---

## Overview

The post is structured as follows, where I try to think through some (I believe) given basics at a high level, but of investing is endlessly complex and this is not financial advice.
...
3. **Factors** - Possibly moving beyond market beta to diversify risk
...
returns_net = returns - pd.Series(daily_cost)
...
![Growth of $1: US vs International Equity Mix](vti_vxus_mix_growth.png)
...
Index funds follow rigid rebalancing rules that create an implicit cost not visible in the expense ratio. Companies typically issue new shares when they believe their stock is overpriced, and buy back shares when they think it's cheap. Because index funds must mechanically buy when firms issue stock and sell when firms buy back, they're effectively "bad market timers", essentially always on the wrong side of the trade. Research suggests this creates roughly a 60 basis point (0.60%) annual performance drag on total market indexes.
...
As an investor who wants to "do slightly better if possible, without fundamentally changing my exposure," DFUS is compelling (DFA, 2024; Felix, 2024). It's basically VTI 2.0, same broad market exposure, just optimized on the margins.
...
```python
# VTI vs DFUS Comparison (Net of Expense Ratios)
import pandas as pd
import numpy as np
...
vti_ret = df.loc[start_date:, 'VTI'].pct_change().dropna()
dfus_ret = df.loc[start_date:, 'DFUS'].pct_change().dropna()

EXPENSE_RATIOS = {'VTI': 0.0003, 'DFUS': 0.0009}  # 0.03% and 0.09%
daily_cost = {k: v / 252 for k, v in EXPENSE_RATIOS.items()}

vti_growth = (1 + vti_ret - daily_cost['VTI']).cumprod()
dfus_growth = (1 + dfus_ret - daily_cost['DFUS']).cumprod()
```
...
![VTI vs DFUS: Growth of $1 Since DFUS Inception](vti_dfus_growth.png)
...
At this point, I was pretty sold on replacing VTI with DFUS for U.S. equity exposure. It's a classic cheap lunch story: essentially the same exposure, potential for slightly better returns, better tax efficiency, for nearly the same low fee. As one wit on the Bogleheads forum put it, "DFUS fixes some academically researched inefficiencies" with index funds.

Alright, U.S. stocks accounted for, I moved to international side, where things were not as straightforward.
...
What about performance and approach? DFAI and DFAE are tilted funds. According to DFA (and Morningstar), DFAI invests in companies of all sizes in developed markets, but with "increased exposure to smaller capitalization, lower relative price, and higher profitability" stocks. Likewise, DFAE targets emerging market stocks with an emphasis on value and profitability. They're not trying to match the index; they're trying to beat it by systematically overweighting those factors. So even though DFA lists these funds under their market funds, they actually do contain factor tilts. Over long periods, I expect they could outperform VXUS (just as DFA's other factor funds have often beaten cap-weight indices historically, but again that is not within the scopt of this post). They will also deviate more from VXUS in the short term.

To be fair, Dimensional does offer one single ETF for international: DFAX (Dimensional World ex-US Core Equity 2 ETF). DFAX bundles developed and emerging in one fund (closer to a VXUS counterpart). But two issues: 

(1) Its expense ratio is ~0.23%, higher than VXUS (because "Core 2" means heavier tilts toward small/value — more trading, more costs). 
...
```python
# VXUS vs DFAX Comparison (Net of Expense Ratios)
import pandas as pd
import numpy as np
...
vxus_ret = df.loc[start_date:, 'VXUS'].pct_change().dropna()
dfax_ret = df.loc[start_date:, 'DFAX'].pct_change().dropna()

EXPENSE_RATIOS = {'VXUS': 0.0007, 'DFAX': 0.0023}  # 0.07% and 0.23%
daily_cost = {k: v / 252 for k, v in EXPENSE_RATIOS.items()}

vxus_growth = (1 + vxus_ret - daily_cost['VXUS']).cumprod()
dfax_growth = (1 + dfax_ret - daily_cost['DFAX']).cumprod()
```
...
![VXUS vs DFAX: Growth of $1 Since DFAX Inception](vxus_dfax_growth.png)

The data shows DFAX modestly outperforming VXUS, about 1.6 percentage points cumulatively, with a CAGR advantage of roughly 0.5% annually. Interestingly, DFAX achieved this with slightly *lower* volatility than VXUS (15.7% vs 16.0%), suggesting the factor tilts provided some diversification benefit. The outperformance is smaller than DFUS vs VTI, which makes sense: DFAX is fighting against a higher expense ratio differential (0.16% vs 0.06%) and the factor premiums have been more muted internationally in this short period.

From a factor perspective: VXUS is basically pure market beta (by design, it's cap-weighted). DFAX is the market beta plus a moderate tilt to value and small. In plain English, DFAX takes more of its risk in smaller and cheaper stocks, and I need to read more personal research being convinced to diversify into these factors. That's been decent recently (2022-2023 were okay for value globally), hence the modest outperformance. But it could lag if large growth stocks roar back or if emerging markets stumble.
...
[Avantis Investors](https://www.avantisinvestors.com/home/) (founded by ex-DFA people), also offers international factor funds but similarly split: AVDE (developed int'l, 0.23% fee) and AVEM (emerging, 0.33% fee). They too expect you to mix your own VXUS cocktail with no simple one-ticker solution there either. Either way, I'm stuck with a two-fund combo for international that is not ideal for multiple reasons.
...
**Malkiel, B. G.** (2019). *A Random Walk Down Wall Street: The Time-Tested Strategy for Successful Investing* (50th Anniversary Edition). W. W. Norton & Company. ISBN: 978-0393358389.  
...
**Felix, B.** Why DFUS Outperformed VTI (And What It Means for Index Funds). *Rational Reminder / YouTube*.  
